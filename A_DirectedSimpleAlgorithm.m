function [Solution] = A_DirectedSimpleAlgorithm(Map,Time,Start,Goal)
a = tic;
clear Solution
...
EndingCriterion = 0;
    ...
    while ( EndingCriterion ~= 1)
          if(Solution(index,1) ~= GoalX && Solution(index,2) ~= GoalY)     % until we hit the straight path to the goal....
              if(Map(Solution(index,2),Solution(index,1)+ 1*sign(GoalX - Solution(index,1))) < Map(Solution(index,2) + 1*sign(GoalY - Solution(index,2)),Solution(index,1)))
